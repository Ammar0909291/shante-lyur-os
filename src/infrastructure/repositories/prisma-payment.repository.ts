...
export class PrismaPaymentRepository implements IPaymentRepository {
  ...
  private toDomain(raw: {
    ...
    return Payment.reconstitute({
      ...
      commissionAmount: raw.commissionAmount
        ? Money.create(Number(raw.commissionAmount), raw.currency)
        : undefined,
      specialistCommission: raw.specialistCommission
        ? Money.create(Number(raw.specialistCommission), raw.currency)
        : undefined,
  ...
  // Port signature: (providerId: string, provider: PaymentProvider)
  ...
  async update(payment: Payment): Promise<Payment> {
    const raw = await this.db.payment.update({
      ...
      data: {
        ...
        failedAt: payment.failedAt,
        failureReason: payment.failureReason,
        commissionAmount: payment.commissionAmount?.amount ?? null,
        specialistCommission: payment.specialistCommission?.amount ?? null,
        metadata: payment.metadata as Prisma.InputJsonValue,
      },
    });
    return this.toDomain(raw);
  ...
  async getRevenueSummary(from: Date, to: Date): Promise<{
    ...
    const payments = await this.db.payment.findMany({
      where: {
        status: { in: ['CAPTURED', 'PARTIALLY_REFUNDED', 'FULLY_REFUNDED'] },
    ...
    for (const p of payments) {
      ...
      totalRevenue += amount;
      byProvider[p.provider] = (byProvider[p.provider] ?? 0) + amount;
