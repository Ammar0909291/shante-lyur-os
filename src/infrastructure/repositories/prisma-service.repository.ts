...
export class PrismaServiceRepository implements IServiceRepository {
  ...
  async findById(id: string): Promise<Service | null> {
    ...
    return raw ? this.toDomain(raw as PrismaService) : null;
  }

  async findByIds(ids: string[]): Promise<Service[]> {
    const raws = await this.db.service.findMany({ where: { id: { in: ids } } });
    return raws.map(r => this.toDomain(r as PrismaService));
  }
  ...
  async findMany(options: {
    ...
    return { items: raws.map(r => this.toDomain(r as PrismaService)), total };
  }
  ...
  async create(service: Service): Promise<Service> {
    ...
    return this.toDomain(raw as PrismaService);
  }
  ...
  async getLocationPrice(serviceId: string, locationId: string): Promise<{ price: number; duration: number } | null> {
    const lp = await this.db.serviceLocationPrice.findFirst({
      where: { serviceId, locationId, isActive: true },
    });
    if (!lp) return null;
    return { price: lp.price.toNumber(), duration: lp.duration };
  }
  ...
  async setLocationPrice(serviceId: string, locationId: string, price: number, duration: number): Promise<void> {
    await this.db.serviceLocationPrice.upsert({
      where: { id: `${serviceId}_${locationId}` },
