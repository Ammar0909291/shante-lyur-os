...
export class PrismaServiceRepository implements IServiceRepository {
  ...
  private toDomain(raw: {
    id: string; name: string; description: string | null; category: unknown;
    basePrice: { toNumber(): number }; baseDuration: number; imageUrl: string | null;
    isActive: boolean; requiresConsultation: boolean; sortOrder: number;
  ...
  async findMany(options?: { category?: ServiceCategory; isActive?: boolean; search?: string; page?: number; limit?: number }): Promise<{ items: Service[]; total: number }> {
    ...
    const [raws, total] = await Promise.all([
      this.db.service.findMany({ where, skip: (page - 1) * limit, take: limit, orderBy: { name: 'asc' } }),
  ...
  async getLocationPrice(serviceId: string, locationId: string): Promise<{ price: number; duration: number } | null> {
    const record = await this.db.serviceLocationPrice.findUnique({
      where: { serviceId_locationId: { serviceId, locationId } },
    });
    if (!record) return null;
    return { price: (record.price as { toNumber(): number }).toNumber(), duration: record.duration };
  }
