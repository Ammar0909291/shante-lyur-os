...
export class PrismaRefundRepository implements IRefundRepository {
  ...
  private toDomain(raw: RawRefund): Refund {
    return new Refund({
      id: raw.id,
  ...
  async findById(id: string): Promise<Refund | null> {
    ...
    return raw ? this.toDomain(raw as unknown as RawRefund) : null;
  }
  ...
  async findByPaymentId(paymentId: string): Promise<Refund[]> {
    ...
    return raws.map(r => this.toDomain(r as unknown as RawRefund));
  }
  ...
  async findMany(options: { status?: RefundStatus; from?: Date; to?: Date; page?: number; limit?: number }): Promise<{ items: Refund[]; total: number }> {
    ...
    return { items: raws.map(r => this.toDomain(r as unknown as RawRefund)), total };
  }
  ...
  async create(refund: Refund): Promise<Refund> {
    ...
    return this.toDomain(raw as unknown as RawRefund);
  }
