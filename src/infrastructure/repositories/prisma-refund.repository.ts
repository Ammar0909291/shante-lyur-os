...
export class PrismaRefundRepository implements IRefundRepository {
  ...
  private toDomain(raw: PrismaRefund): Refund {
  ...
  async findById(id: string): Promise<Refund | null> {
    ...
    return raw ? this.toDomain(raw as PrismaRefund) : null;
  }
  ...
  async findByPaymentId(paymentId: string): Promise<Refund[]> {
    ...
    return raws.map(r => this.toDomain(r as PrismaRefund));
  }
  ...
  async findMany(options: {
    ...
    const { status, from, to, page = 1, limit = 50 } = options;
    ...
    return { items: raws.map(r => this.toDomain(r as PrismaRefund)), total };
  }
  ...
  async create(refund: Refund): Promise<Refund> {
    ...
    return this.toDomain(raw as PrismaRefund);
  }
