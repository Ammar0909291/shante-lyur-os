...
export class PrismaCustomerProfileRepository implements ICustomerProfileRepository {
  ...
  private toDomain(raw: RawCustomerProfile): CustomerProfile {
    return CustomerProfile.reconstitute({
      ...
      totalSpent: Money.create(raw.totalSpent.toNumber(), 'RUB'),
  ...
  async findById(id: string): Promise<CustomerProfile | null> {
    ...
    return raw ? this.toDomain(raw as RawCustomerProfile) : null;
  }
  ...
  async findMany(options: {
    ...
    const { loyaltyTier, minVisits, page = 1, limit = 20 } = options;
    ...
    return { items: raws.map(r => this.toDomain(r as RawCustomerProfile)), total };
  }
  ...
  async create(profile: CustomerProfile): Promise<CustomerProfile> {
    ...
    return this.toDomain(raw as RawCustomerProfile);
  }
  ...
  async updateChurnRisk(userId: string, score: number): Promise<void> {
    await this.db.customerProfile.updateMany({
      where: { userId },
      data: { churnRiskScore: score, updatedAt: new Date() },
    });
  }
  ...
  async getRetentionMetrics(): Promise<{
    ...
    const [totalCustomers, activeCustomers, avgResult] = await Promise.all([
    ...
    return {
      totalCustomers,
      activeCustomers,
      atRiskCustomers: Math.max(0, totalCustomers - activeCustomers),
      avgLifetimeValue: Number(avgResult._avg.totalSpent ?? 0),
    };
  }
}
