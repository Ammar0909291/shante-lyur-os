...
type RawAppointment = {
  ...
  createdAt: Date;
  updatedAt: Date;
  services?: Array<{ serviceId: string; price: number | { toNumber(): number }; duration: number; sortOrder: number; service?: { name: string } | null }>;
};
...
export class PrismaAppointmentRepository implements AppointmentRepositoryPort {
  ...
  private toDomain(raw: RawAppointment): Appointment {
    const services: AppointmentServiceItem[] = (raw.services ?? []).map(s => ({
      ...
      name: s.service?.name ?? '',
      price: Money.create(this.toNum(s.price)),
      duration: s.duration,
    ...
    return new Appointment({
      ...
      services,
      totalPrice: Money.create(this.toNum(raw.totalPrice)),
  ...
  async findByCustomerId(customerId: string, page?: number, limit?: number): Promise<{ items: Appointment[]; total: number }> {
    ...
    const [raws, total] = await Promise.all([
      ...
      this.db.appointment.count({ where: { clientId: customerId } }),
    ]);
  ...
  async findOverlapping(specialistId: string, timeRange: DateRange, excludeId?: string): Promise<Appointment[]> {
    const startAt = timeRange.start;
    const endAt = timeRange.end;
    const where: Prisma.AppointmentWhereInput = {
      specialistId,
