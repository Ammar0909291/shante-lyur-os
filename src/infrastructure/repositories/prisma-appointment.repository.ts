...
type RawAppointment = {
  ...
  createdAt: Date;
  updatedAt: Date;
  services?: Array<{ serviceId: string; price: { toNumber(): number }; duration: number; sortOrder: number; service?: { name: string } | null }>;
};

export class PrismaAppointmentRepository implements IAppointmentRepository {
  constructor(private readonly db: PrismaClient) {}

  private toDomain(raw: RawAppointment): Appointment {
    const services: AppointmentServiceItem[] = (raw.services ?? []).map(s => ({
      serviceId: s.serviceId,
      name: s.service?.name ?? '',
      price: Money.create(s.price.toNumber(), 'RUB'),
      duration: s.duration,
    ...
    return Appointment.reconstitute({
      ...
      services,
      totalPrice: Money.create(raw.totalPrice.toNumber(), 'RUB'),
  ...
  async findById(id: string): Promise<Appointment | null> {
    ...
    return raw ? this.toDomain(raw as unknown as RawAppointment) : null;
  }
  ...
  async findByDateRange(start: Date, end: Date, filters?: { specialistId?: string; locationId?: string; status?: AppointmentStatus[] }): Promise<Appointment[]> {
    ...
    return raws.map(r => this.toDomain(r as unknown as RawAppointment));
  }

  async findByCustomerId(clientId: string, page?: number, limit?: number): Promise<{ items: Appointment[]; total: number }> {
    ...
    const [raws, total] = await Promise.all([
      ...
      this.db.appointment.count({ where: { clientId } }),
    ]);
    return { items: raws.map(r => this.toDomain(r as unknown as RawAppointment)), total };
  }
  ...
  async create(appointment: Appointment): Promise<Appointment> {
    ...
    return this.toDomain(raw as unknown as RawAppointment);
  }
