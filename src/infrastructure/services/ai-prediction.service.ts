...
export class AIPredictionService implements IAIPredictionService {
  ...
  async forecastRevenue(days: number): Promise<{ date: string; predictedRevenue: number; confidence: number }[]> {
    ...
    const { items } = await this.revenueRepo.findMany({ from: start, to: end, limit: 10000 });
    const avgDaily = items.length > 0
      ? items.reduce((sum, h) => sum + h.amount.amount, 0) / items.length
      : 0;
