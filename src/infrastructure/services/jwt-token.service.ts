...
export class JwtTokenService implements ITokenService {
  ...
  private readonly ACCESS_EXPIRY  = 15 * 60;        // 15 min (seconds)
  private readonly REFRESH_EXPIRY = 7 * 24 * 3600;  // 7 days (seconds)

  constructor() {
    const base = process.env.JWT_SECRET ?? 'dev-access-secret-change-me';
    this.ACCESS_SECRET  = process.env.JWT_ACCESS_SECRET  ?? base;
    this.REFRESH_SECRET = process.env.JWT_REFRESH_SECRET ?? base;
  }
