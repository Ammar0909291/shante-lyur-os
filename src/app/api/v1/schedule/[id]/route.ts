...
export async function DELETE(
  ...
  const existing = await prisma.workingSchedule.findUnique({ where: { id: params.id } });
  if (!existing) return R.notFound('Schedule not found');

  await prisma.workingSchedule.delete({ where: { id: params.id } });
