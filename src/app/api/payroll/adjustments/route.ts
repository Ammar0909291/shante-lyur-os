...
import { AddPayrollAdjustmentUseCase } from '@/application/use-cases/payroll';
import { AddPayrollAdjustmentSchema } from '@/application/dto';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = AddPayrollAdjustmentSchema.safeParse(body);
    if (!parsed.success) return apiError('VALIDATION_ERROR', 'Invalid request', 400, { issues: parsed.error.issues });

    const registry = DIRegistry.instance;
    const useCase = new AddPayrollAdjustmentUseCase(registry.payrollRepository);
