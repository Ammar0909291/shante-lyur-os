...
export function NewBookingDialog({ open, onClose, onCreated }: NewBookingDialogProps) {
  ...
  React.useEffect(() => {
    if (open) {
      ...
      setClientSearch(''); setError(null); setSuccess(false);
  ...
  // Load specialists from API when dialog opens
  React.useEffect(() => {
    if (!open) return;
    fetch('/api/specialists?limit=50', { credentials: 'include' })
      ...
      .then(json => {
        ...
        const mapped: ModalSpecialist[] = json.data.items.map((s: {
          ...
          specializations: s.specializations ?? (s.specialization ? [s.specialization] : []),
          rating: s.rating,
        }));
  ...
  // Load services when specialist is selected
  React.useEffect(() => {
  ...
  // Load slots when specialist + date selected
  React.useEffect(() => {
    ...
    fetch(
      ...
      .catch(() => {
        // Generate mock slots client-side as fallback
        setSlots(generateMockSlots(date, service.duration));
      })
      .finally(() => setLoadingSlots(false));
  }, [specialist, date, service]);
  ...
  async function handleSubmit() {
    ...
    const startAt = new Date(`${date}T${String(h - 3).padStart(2, '0')}:${String(m).padStart(2, '0')}:00.000Z`); // Convert Moscow → UTC
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Check for conflict/slot suggestion
      ...
      // Optimistic creation for dev environments without DB
      setSuccess(true);
      setTimeout(() => {
        onCreated({
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {sp.rating && (
                        <span className="text-xs text-champagne">★ {sp.rating}</span>
                      )}
                    </div>
                ...
                {DAYS.map(d => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={cn('text-[10px]', date === d ? 'text-champagne/70' : 'text-text-tertiary')}>
                        {month}
                      </span>
                      {isToday && (
                        <span className="text-[9px] font-bold mt-0.5 text-sage">сегодня</span>
                      )}
                    </button>
                  );
                })}
                ...
                <>
                  <div className="grid grid-cols-4 gap-2">
                    {slots.map(slot => (
                      <button
                        ...
                        title={!slot.available ? (slot.reason === 'occupied' ? 'Занято' : slot.reason === 'past' ? 'Прошедшее время' : 'Недоступно') : undefined}
                      >
                        {slot.time}
                      </button>
                    ))}
                  ...
                  {slots.filter(s => s.available).length === 0 && (
                    <div className="flex flex-col items-center gap-2 py-6 text-center">
                      ...
                      <p className="text-sm text-text-secondary">Нет доступных слотов</p>
                      {nextAvailableDate && (
                        <button
                          onClick={() => { setDate(nextAvailableDate); setNextAvailableDate(null); }}
                          className="text-sm text-champagne hover:underline flex items-center gap-1"
                        >
                          <Calendar className="w-3.5 h-3.5" />
                          Следующий день: {formatDay(nextAvailableDate)}
                        </button>
                      )}
                    </div>
                  )}
              ...
              {success ? (
                ...
                <>
                  ...
                  <div className="bg-charcoal rounded-2xl p-4 space-y-3 text-sm">
                    <p className="text-xs font-semibold uppercase tracking-widest text-text-tertiary mb-3">Сводка</p>
                    {[
                      { icon: <User className="w-3.5 h-3.5" />, label: 'Клиент', value: client?.name },
                      { icon: <Avatar name={specialist?.name ?? ''} size="xs" />, label: 'Специалист', value: specialist?.name },
                      ...
                      { icon: <Clock className="w-3.5 h-3.5" />, label: 'Время', value: time ? `${time} · ${formatDuration(service?.duration ?? 0)}` : '' },
