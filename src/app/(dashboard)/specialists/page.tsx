...
export default function SpecialistsPage() {
  ...
  const [specialists, setSpecialists] = React.useState(mockSpecialists);

  function toggleActive(id: string) {
    setSpecialists((prev) => prev.map((s) => s.id === id ? { ...s, active: !s.active } : s));
  }
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-serif text-2xl font-medium text-text-primary tracking-tight">Специалисты</h2>
          <p className="text-sm text-text-secondary mt-0.5">{specialists.length} специалиста в студии</p>
        </div>
        <Button
          variant="primary"
          size="md"
          ...
          >
            <div className="flex items-start justify-between gap-4 mb-5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-serif text-lg font-medium text-text-primary leading-tight">{spec.name}</h3>
                  <p className="text-sm text-text-secondary mt-0.5">{spec.title}</p>
                  <div className="flex items-center gap-3 mt-1.5">
                    <StarRating rating={spec.rating} />
                    <button onClick={() => toggleActive(spec.id)} title={spec.active ? 'Деактивировать' : 'Активировать'}>
            ...
            <div className="grid grid-cols-3 gap-3 mb-5">
              ...
              <div className="bg-charcoal rounded-xl p-3 text-center">
                ...
                <p className="font-serif text-base font-medium text-text-primary leading-tight">{formatCurrency(spec.revenueMtd)}</p>
            ...
            <div className="mb-5">
              <p className="text-xs font-semibold uppercase tracking-widest text-text-tertiary mb-2">Специализации</p>
            ...
            <div className="pt-4 border-t border-border-luxury flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="secondary" size="sm" onClick={() => toggleActive(spec.id)}>
