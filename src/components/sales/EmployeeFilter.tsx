...
export function EmployeeFilter({ selected, onChange, className }: EmployeeFilterProps) {
  ...
  React.useEffect(() => {
    ...
    fetch('/api/specialists?limit=100')
      .then((r) => r.json())
      .then((json) => {
        const items = json?.data?.specialists ?? json?.data?.items ?? [];
  ...
  const label =
    selected.length === 0  ? 'Все сотрудники'
    : selected.length === 1 ? (options.find((o) => o.id === selected[0])?.name ?? '1 сотрудник')
